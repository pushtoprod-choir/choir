...
`trips` (`id`, `chat_id`, `started_by`, `title`, `started_at`, `ended_at`, `status`, `summary`) links back from `negotiations` via a nullable `trip_id` column (added the `ALTER TABLE` way, since `negotiations` predates trips). `status` is `'active'` or `'ended'`; `get_active_trip` finds the most recent `'active'` row per chat.
